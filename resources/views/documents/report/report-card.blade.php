...
@section('content')
    ...
    @foreach ($students as $student)
        ...
        <div style="margin-left: 10em; padding-top: 5rem;">
            ...
            <span
                style="font-size: 15px; text-align: center ">{{ $student->current_grade_id == 1 ? '1er' : ($student->current_grade_id == 2 ? '2º' : ($student->current_grade_id == 3 ? '3º' : '')) }}
                GRADO DE EDUCACIÓN SECUNDARIA CICLO ESCOLAR</span> <strong>   {{ $student->date->copy()->subYear()->year }} - {{ $student->date->year }}</strong> <br />
        ...
        <div style="margin-top: 1rem;">
            ...
            <div class="row" style="margin-top: 1rem;">
                <div class="column" style="position: relative;">
                    <span class="bold-text"{{ strlen($student->user->lastname1) > 8 ?: 'style="font-size: 9px"' }}> {{ strtoupper($student->user->lastname1) }} </span>
                    <div style="position: absolute; left:0px; top:0px; margin-top: 20px;">PRIMER APELLIDO</div>
                </div>
                <div class="column" style="position: relative;">
                    <span class="bold-text" style="width 50px; {{ strlen($student->user->lastname1) < 12 ?: 'font-size: 10px;' }}>"> {{ strtoupper($student->user->lastname2) }} </span>
                    <div style="position: relative; left: 0px; margin-top: 0px;">SEGUNDO APELLIDO</div>
                </div>
                <div class="column" style="position: relative;">
                    <span class="bold-text"> {{ strtoupper($student->user->name) }} </span>
                    <div style="position: absolute; left:0px; top:0px; margin-top: 20px;">NOMBRE(S)</div>
                </div>
                <div class="column" style="position: relative;">
                    <span class="bold-text"> {{ $student->user->curp }} </span>
        ...
        <div class="row" style="font-size: 14px;">
            ...
            <div style="float: left;
                            width: 15%;">
                <span> {{ $student->currentGroup->name }} </span> <br />
                    ...
                    <tbody id="cetrar">
                        ...
                        <tr>
                            <td>LENGUA
                                MATERNA
                                (ESPAÑOL)</td>
                            <td> {{ $student->DGIQ5B70YO }} </td>
                            <td> {{ $student->DFVH73TQKX }} </td>
                            <td> {{ $student->{'9OFPYCD1RA'} }} </td>
                            <td> {{ !empty($student->{'9OFPYCD1RA'}) ? round(($student->{'9OFPYCD1RA'} + $student->DFVH73TQKX + $student->DGIQ5B70YO) / 3, 2) : '---' }}
                            </td>
                        </tr>
                        <tr>
                            <td>MATEMÁTICAS</td>
                            <td> {{ $student->RBVHGUZOKN }} </td>
                            <td> {{ $student->R8V5G6LXSI }}</td>
                            <td> {{ $student->ZT8VG4OXP0 }}</td>
                            <td> {{ !empty($student->ZT8VG4OXP0) ? round(($student->ZT8VG4OXP0 + $student->R8V5G6LXSI + $student->RBVHGUZOKN) / 3, 2) : '---' }}
                            </td>
                        </tr>
                        <tr>
                            <td>LENGUA EXTRANJERA (INGLÉS)</td>
                            <td> {{ $student->VBFXR9LKGZ }} </td>
                            <td> {{ $student->EJA208KHIW }}</td>
                            <td> {{ $student->ZB89KETHAQ }}</td>
                            <td> {{ !empty($student->ZB89KETHAQ) ? round(($student->ZB89KETHAQ + $student->EJA208KHIW + $student->VBFXR9LKGZ) / 3, 2) : '---' }}
                            </td>
                        </tr>
                        <tr>
                            <td>CIENCIAS (BIOLOGÍA)</td>
                            <td> {{ $student->O8XNPFKZJ2 }} </td>
                            <td> {{ $student->AFLTZS57CQ }}</td>
                            <td> {{ $student->LVWUYJE1H6 }}</td>
                            <td> {{ !empty($student->LVWUYJE1H6) ? round(($student->LVWUYJE1H6 + $student->AFLTZS57CQ + $student->O8XNPFKZJ2) / 3, 2) : '---' }}
                            </td>
                        </tr>
                        <tr>
                            <td>HISTORIA</td>
                            <td> -- </td>
                            <td> {{ $student->J72AEBQZ3S }}</td>
                            <td> {{ $student->A0J8KU3YTZ }}</td>
                            <td> {{ !empty($student->A0J8KU3YTZ) ? round(($student->A0J8KU3YTZ + $student->J72AEBQZ3S) / 2, 2) : '---' }}
                            </td>
                        </tr>
                        <tr>
                            <td>GEOGRAFÍA</td>
                            <td> {{ $student->M7ODHBWX30 }} </td>
                            <td> -- </td>
                            <td> -- </td>
                            <td> {{ $student->M7ODHBWX30 }}</td>
                        </tr>
                        <tr>
                            ...
                            <td> {{ $student->QIXSZ9NFWV }} </td>
                            <td> {{ $student->YH4PL7CZT6 }}</td>
                            <td> {{ $student->HSEMVIT39N }}</td>
                            <td> {{ !empty($student->HSEMVIT39N) ? round(($student->HSEMVIT39N + $student->YH4PL7CZT6 + $student->QIXSZ9NFWV) / 3, 2) : '---' }}
                            </td>
                        </tr>
                        <tr>
                            <td>TECNOLOGÍA</td>
                            <td> {{ $student->PSXQ0LZ1AT }} </td>
                            <td> {{ $student->HBIQ03KYW1 }}</td>
                            <td> {{ $student->D2NGLBPSHC }}</td>
                            <td> {{ !empty($student->D2NGLBPSHC) ? round(($student->D2NGLBPSHC + $student->HBIQ03KYW1 + $student->PSXQ0LZ1AT) / 3, 2) : '---' }}
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td> {{ $student->WJ4S6LH0PV }} </td>
                            <td> {{ $student->{'9MGZI8RSXH'} }}</td>
                            <td> {{ $student->{'4ISL6YXD3M'} }}</td>
                            <td> {{ !empty($student->{'4ISL6YXD3M'}) ? round(($student->{'4ISL6YXD3M'} + $student->{'9MGZI8RSXH'} + $student->WJ4S6LH0PV) / 3, 2) : '---' }}
                            </td>
                        </tr>
                        <tr>
                            <td>ARTES</td>
                            {{-- ACCEDER A UNA KEY QUE INICIE CON UN ENTERO EN EL INICIO --}}
                            <td> {{ $student->{'0XLUGHMA8F'} }} </td>
                            <td> </td>
                            <td> {{ $student->{'7Y425ONT8K'} }}</td>
                            <td> {{ !empty($student->{'7Y425ONT8K'}) ? round(($student->{'7Y425ONT8K'} + $student->{'0XLUGHMA8F'}) / 2, 2) : '---' }}
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div style="margin-left:320pt;">
                ...
                <div class="sello-container separar">
                    ...
                    <div class="fecha-sello">
                        <span class="sello-fiels"> {{ date('Y') }} </span>
                        <span class="sello-fiels">{{ date('m') }}</span>
                        <span class="sello-fiels">{{ date('d') }}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="page-break"></div>
        @include('documents.report.report-page-two')
    @endforeach
@endsection
<script>
