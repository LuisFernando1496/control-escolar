...
<div class="flex bg-white shadow-lg rounded-lg  relative"> 
    ...
    <div class="flex items-center bg-blue-100 rounded-tr-3xl">
        <x-word-svg color="0288D1" height="10" />
    </div>
    <div class="flex items-start px-4 py-11 sm:py-6">
        <div class="">
            <div class="flex items-center justify-between ">
                <h3 class="font-semibold mb-2 text-xl leading-tight sm:leading-normal block"> {{ $asset->title }} </h3>
            </div>
            ...
            <div class="mt-4 flex items-center">
                <div class="flex mr-2 text-gray-700 bg-blue-100 rounded-md px-2 hover:bg-blue-300 text-sm mr-3">
                    ...
                    {{-- route to download a file sotre in the server--}}
                    <a href="{{ route('asset.download', $asset->path) }}">descargar</a>
                </div>
                <div class="flex mr-2 text-gray-700 text-sm mr-4 bg-blue-100 rounded-md px-2 hover:bg-blue-300">
                    ...
                    {{-- route to view  only pdf file or an image --}}
                    <a href="{{ route('asset.show', $asset->path) }}" target="_blank">abrir</a>
                </div>
            </div>
        </div>
    </div>
    <div
        ...
        <span> {{ date('Y/m/d', strtotime($asset->created_at)) }} </span>
    </div>
</div>
